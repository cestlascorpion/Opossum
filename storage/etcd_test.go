...
func TestNewEtcdHolder(t *testing.T) {
	holder, err := NewEtcdHolder(context.Background(), "192.168.35.50", "9090", []string{"127.0.0.1:2379"}, "for_test", 1023)
	if err != nil {
		fmt.Println(err)
		t.FailNow()
	}
...
func TestNewEtcdHolder2(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		h, err := NewEtcdHolder(context.Background(), "192.168.35.50", fmt.Sprintf("%d", 9090+i), []string{"127.0.0.1:2379"}, "for_test", 1023)
		if err != nil {
			fmt.Println(err)
			t.FailNow()
		}
